 ...
 *
 * Returns voicemail messages with optional filtering.
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase
      .from('communications')
      .select(
        'id, from_address, from_name, body, status, direction, duration, recording_url, twilio_call_sid, created_at, customer:customers!customer_id(id, name, email, phone)',
        { count: 'exact' }
      )
      .eq('type', 'voicemail')
